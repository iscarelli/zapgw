...
| A handler declares which instance types it accepts | a **mandatory positional** parameter: omitting it **does not compile**, and the zero value is the most restrictive one | ✅ **here** — `internal/outbound/tipos.go` (`AcceptedTypes`, T-111): every `outbound.New*Handler` constructor takes it as its LAST positional argument (e.g. `NewReadsHandler`, `internal/outbound/leituras_handler.go:65`). The zero value, `WhatsAppOnly` (`iota` = 0), rejects Instagram rather than accepting everything, so an accidentally-zeroed value fails closed. **Proved on 2026-08-31:** removed `outbound.WhatsAppOnly` from the `NewReadsHandler` call in `cmd/zapgw/main.go:430` and ran `go build ./...` — `not enough arguments in call to outbound.NewReadsHandler … want (…, outbound.AcceptedTypes)`; restored, `git diff` empty. T-197 settled this: the earlier "not located" sweep missed it because the parameter lives in `internal/outbound/`, not in a file named after the rule |
| A string the deploy scripts `grep` from the gateway's output stays in sync with the Go that emits it | a **coupling gate** that reads every `deploy/*.sh`, extracts the literals marked `# zapgw:log-coupling "…"`, and requires each to still exist under `cmd/` or `internal/` — with **zero markers found** being a hard *could not verify*, never a pass | ✅ **here** (`internal/config/shell_log_coupling_test.go`, T-235). 🔴 **It has failed against real data twice on 2026-09-07** — once by the implementer, once re-proved by the planner instead of taken from the report: reverting one marker to its old Portuguese wording makes it print `implanta/deploy.sh:268 greps "esta obsoleta -- use", which no longer appears anywhere in cmd/ or internal/` (that run's actual output, from before `implanta/` was renamed to `deploy/` at T-230), and it names **both** valid fixes so the reader is not nudged into silencing it. A permanent positive-control subtest re-proves the mechanism on a throwaway tree every run. **Why this rule exists:** `go test ./...` does not read `.sh`, so this seam had *no* mechanism at all — and two couplings broke through it, from two different tasks (T-219 and T-224), the second one silently deleting the obsolete-env-var warning from every deploy. *A `grep` that stops matching returns nothing, and nothing is what a healthy deploy looks like.* |
...
| A secret never in the repository | `.gitignore` since the initial commit; a per-project hook | partial — the `.gitignore` is here. A **pre-push hook now exists** (`.githooks/pre-push`, T-199) — but it covers a narrower thing than "secrets in general": it blocks the two personal-data gates (phone number, customer name) over the INTERVAL being pushed, not just the final tree, so a number a later commit deletes still gets caught before it reaches `origin`. It does **not** cover an API token or key in a commit — that stays only the `.gitignore`'s job. Activate with `git config core.hooksPath .githooks` (a per-clone command — nothing commits this state); confirm with `git config --get core.hooksPath` |
