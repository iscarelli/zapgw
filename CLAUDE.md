...
| The verify runs before every merge | **CI** with the four commands in separate steps, `timeout-minutes: 10`, and no step piping into `grep` | 🔴 **does not exist anywhere today, and comes back on 2026-09-01.** 🔴 **This repository is the ONLY place where it comes back into existence**, born together with the first code — the full account is in *The three that have no mechanism anywhere*, below |
