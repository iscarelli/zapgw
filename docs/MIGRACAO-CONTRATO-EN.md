...
`internal/outbound/input_aliases.go`; the old-name counter
...
accepts (`internal/outbound/message.go:179` and `:626`) both already emit/accept **`media_id`**,
...
| 11 | `botoes_url` | `url_buttons` | ENTRADA | internal/outbound/message.go:549 (field kept only to be REFUSED, T-045) |
...
| 15 | `categoria` | `category` | SAIDA-EVENTO + ENTRADA + SAIDA-RESPOSTA | internal/meta/types.go:181,235 (event); internal/outbound/message.go:627 (request); internal/outbound/templates_handler.go:363,390 (response) |
...
| 40 | `estado` | `state` | SAIDA-EVENTO + SAIDA-RESPOSTA | internal/meta/types.go:242,352,398 (event); internal/outbound/state.go:66 and others (response) |
...
| 44 | `idioma` | `language` | SAIDA-EVENTO + ENTRADA + SAIDA-RESPOSTA | internal/meta/types.go:230,295 (event); internal/outbound/message.go:522 (request); internal/outbound/templates_handler.go:364,543 (response) |
| 45 | `instancia` | `instance` | ENTRADA + SAIDA-RESPOSTA + SAIDA-EVENTO | internal/outbound/message.go:512 (request); internal/outbound/state.go:44 (response); internal/inbound/deliver.go:45 (event envelope) |
| 46 | `instancias` | `instances` | A MEDIR | not a JSON contract key today — only exists as a CLI flag (`--instancias`), outside HTTP (cmd/zapgw/provision.go:1476) |
...
| 55 | `motivo` | `reason` | SAIDA-EVENTO + SAIDA-RESPOSTA | internal/meta/types.go:251 (event); internal/outbound/leadership.go:231 (response) |
| 56 | `nome` | `name` | SAIDA-EVENTO + ENTRADA + SAIDA-RESPOSTA | internal/meta/types.go:229,294 (event); internal/outbound/message.go:330,485 (request); internal/outbound/templates_handler.go:362,552 (response) |
| 57 | `nome_arquivo` | `file_name` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:581 (event); internal/outbound/message.go:181,629 (request) |
| 58 | `nome_contato` | `contact_name` | SAIDA-EVENTO | internal/meta/types.go:445 |
| 59 | `numero_exibido` | `display_number` | SAIDA-EVENTO + ENTRADA + SAIDA-RESPOSTA | internal/meta/types.go:345 (event); internal/outbound/registration_handler.go:96 (request); internal/outbound/health_handler.go:84 (response) |
...
| 77 | `template` | `template` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:617 (event); internal/outbound/message.go:521 (request) — same spelling in both languages, no rename needed |
...
| 80 | `texto` | `text` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:446 (event); internal/outbound/message.go:177,239,518 (request) |
| 81 | `tipo` | `kind` | SAIDA-EVENTO + ENTRADA + SAIDA-RESPOSTA | internal/meta/types.go:413 (event); internal/outbound/message.go:514 (request); internal/outbound/state.go:51 (response) |
...
| 90 | `via` | `via` | SAIDA-RESPOSTA | internal/outbound/ingress.go:212 — same spelling in both languages |
...
| # | portuguese | english | direction | file:line |
|---|---|---|---|---|
| 91 | `alvo` | `target` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:68 (event); internal/outbound/message.go:309 (request) |
...
| 101 | `emoji` | `emoji` (unchanged) | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:67 (event); internal/outbound/message.go:313 (request) |
| 102 | `endereco` | `address` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:89 (event); internal/outbound/message.go:331 (request) |
...
| 108 | `legenda` | `caption` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:580 (event); internal/outbound/message.go:628 (request) |
| 109 | `localizacao` | `location` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:608 (event); internal/outbound/message.go:640 (request) |
...
| 113 | `reacao` | `reaction` | SAIDA-EVENTO + ENTRADA | internal/meta/types.go:541 (event); internal/outbound/message.go:637 (request) |
...
(`internal/outbound/message.go:271-274`), so the same word is a real field in both an outgoing
...
- `instancias` — only exists as a CLI flag (`--instancias`, `cmd/zapgw/provision.go:1476`), never
