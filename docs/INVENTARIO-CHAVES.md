Código: internal/meta/types.go, internal/meta/templates.go, internal/meta/parse.go, internal/meta/profile.go,
internal/outbound/handler.go, internal/outbound/message.go, internal/outbound/state.go,
internal/outbound/watchdog.go, internal/outbound/external_probe.go, internal/outbound/ingress.go,
internal/outbound/block_handler.go, internal/outbound/templates_handler.go,
internal/outbound/health_handler.go, internal/outbound/smoke_handler.go,
internal/outbound/media_handler.go, internal/outbound/profile_handler.go, internal/inbound/deliver.go
...
`block_handler.go`, `registration_handler.go`, `state_handler.go`, `smoke_handler.go`,
`handler.go`, `reads_handler.go`, `leadership.go`, `media_handler.go`, `pause_handler.go`,
`profile_handler.go`, `health_handler.go`, `templates_handler.go`, `types.go` — ou seja, é o corpo
...
| 1 | `alvo` | `internal/outbound/message.go:309` | `ReactionRequest` (em `Request.reacao`) | ENTRADA | `POST /v1/messages` |
| 2 | `bloqueados` | `internal/outbound/block_handler.go:173` | `blockListResponse` | SAIDA-RESPOSTA | `GET /v1/bloqueios` |
| 3 | `certificado_do_callback` | `internal/outbound/state.go:174` | `State` | SAIDA-RESPOSTA | `GET /v1/estado` |
| 4 | `checagem_falhando_desde` | `internal/outbound/watchdog.go:145` | `MetaToken` (em `State.token_meta`) | SAIDA-RESPOSTA | `GET /v1/estado` |
...
| 7 | `codigo_meta` | `internal/outbound/block_handler.go:146` | `blockFailureResponse` | SAIDA-RESPOSTA | `POST/DELETE /v1/bloqueios` |
...
| 9 | `detalhe_meta` | `internal/outbound/block_handler.go:148` | `blockFailureResponse` | SAIDA-RESPOSTA | `POST/DELETE /v1/bloqueios` |
...
| 11 | `emoji` | `internal/outbound/message.go:313` | `ReactionRequest` (em `Request.reacao`) | ENTRADA | `POST /v1/messages` |
| 12 | `endereco` | `internal/meta/types.go:89` | `Location` (em `Event.localizacao`) | SAIDA-EVENTO | webhook (`POST callback_url`) |
| 12 | `endereco` | `internal/outbound/message.go:331` | `LocationRequest` (em `Request.localizacao`) | ENTRADA | `POST /v1/messages` |
| 13 | `explicacao_meta` | `internal/outbound/handler.go:297` | `errorResponse.Error` | SAIDA-RESPOSTA | erro compartilhado (ver nota acima) |
| 14 | `falhando_desde` | `internal/outbound/ingress.go:201` | `ConnectorInState` (em `State.entrada.conector`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 14 | `falhando_desde` | `internal/outbound/state.go:543` | `InstagramTokenInState` (em `State.token_instagram`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 15 | `fonte` | `internal/outbound/state.go:443` | `ObservedValue` (em `State.numero_na_meta`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 15 | `fonte` | `internal/outbound/external_probe.go:162` | `ExternalReachInState` (em `State.alcance_externo`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 16 | `gerado_em` | `internal/outbound/state.go:86` | `State` | SAIDA-RESPOSTA | `GET /v1/estado` |
| 17 | `instrucao` | `internal/outbound/state.go:547` | `InstagramTokenInState` (em `State.token_instagram`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 18 | `legenda` | `internal/meta/types.go:580` | `Event` | SAIDA-EVENTO | webhook (`POST callback_url`) |
| 18 | `legenda` | `internal/outbound/message.go:628` | `Request` | ENTRADA | `POST /v1/messages` |
| 19 | `localizacao` | `internal/meta/types.go:608` | `Event` | SAIDA-EVENTO | webhook (`POST callback_url`) |
| 19 | `localizacao` | `internal/outbound/message.go:640` | `Request` | ENTRADA | `POST /v1/messages` |
| 20 | `medido_em` | `internal/outbound/ingress.go:194` | `ConnectorInState` (em `State.entrada.conector`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 20 | `medido_em` | `internal/outbound/external_probe.go:159` | `ExternalReachInState` (em `State.alcance_externo`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 20 | `medido_em` | `internal/outbound/watchdog.go:143` | `MetaToken` (em `State.token_meta`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 21 | `processados` | `internal/outbound/block_handler.go:158` | `blockOperationResponse` | SAIDA-RESPOSTA | `POST/DELETE /v1/bloqueios` |
...
| 23 | `reacao` | `internal/outbound/message.go:637` | `Request` | ENTRADA | `POST /v1/messages` |
| 24 | `subcodigo_meta` | `internal/outbound/handler.go:293` | `errorResponse.Error` | SAIDA-RESPOSTA | erro compartilhado (ver nota acima) |
| 25 | `token_instagram` | `internal/outbound/state.go:192` | `State` | SAIDA-RESPOSTA | `GET /v1/estado` |
| 26 | `token_meta` | `internal/outbound/state.go:169` | `State` | SAIDA-RESPOSTA | `GET /v1/estado` |
| 27 | `valor` | `internal/outbound/state.go:432` | `ObservedValue` (em `State.numero_na_meta`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 28 | `veredito` | `internal/outbound/state.go:513` | `InstagramTokenInState` (em `State.token_instagram`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 28 | `veredito` | `internal/outbound/health_handler.go:94` | `healthResponse` | SAIDA-RESPOSTA | `GET /v1/instances/{slug}/health` |
| 28 | `veredito` | `internal/outbound/external_probe.go:152` | `ExternalReachInState` (em `State.alcance_externo`) | SAIDA-RESPOSTA | `GET /v1/estado` |
| 28 | `veredito` | `internal/outbound/watchdog.go:142` | `MetaToken` (em `State.token_meta`) | SAIDA-RESPOSTA | `GET /v1/estado` |
...
- 6 chaves aparecem em SAIDA-EVENTO **e** ENTRADA com o mesmo nome, por desenho (`alvo`, `emoji`,
  ...
  deliberadamente o mesmo dos dois lados, ver comentário em `message.go:271-274`).
...
- structs internos que decodificam o formato CRU da Meta e morrem por dentro (nunca chegam ao
  ...
  (`parse.go:361-376`), os decoders de `block.go`, `instagram.go`, `client.go`, `number.go`,
  `instagram_diagnostics.go`;
- `internal/outbound/ingress.go:431` (`readyConnections`, camelCase) — não é nosso contrato: é
