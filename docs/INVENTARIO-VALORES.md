Código: internal/config/counter.go, internal/config/store.go, internal/meta/types.go,
internal/meta/media.go, internal/meta/errors.go, internal/outbound/message.go,
internal/outbound/state.go, internal/outbound/state_handler.go, internal/outbound/ingress.go,
internal/outbound/external_probe.go, internal/outbound/leadership.go, internal/outbound/watchdog.go,
internal/outbound/health_handler.go, internal/outbound/handler.go,
internal/outbound/block_handler.go, internal/outbound/media_handler.go,
internal/outbound/templates_handler.go, internal/outbound/profile_handler.go
...
4. A **count of every counter that exists in the code today**, against the closed vocabulary of
   counter constants (`internal/config/counter.go`).
...
Direction: **ENTRADA**. Field: `tipo`, `internal/outbound/message.go:514` (json tag, see
`docs/MIGRACAO-CONTRATO-EN.md` row 81). Values enforced by the `switch` in `Request.Validate`:

| value | arquivo:linha | english |
|---|---|---|
| `texto` | internal/outbound/message.go:749 | A DECIDIR |
| `template` | internal/outbound/message.go:753 | A DECIDIR |
| `botoes` | internal/outbound/message.go:774 | A DECIDIR |
| `cta_url` | internal/outbound/message.go:809 | A DECIDIR |
| `lista` | internal/outbound/message.go:825 | A DECIDIR |
| `pedir_localizacao` | internal/outbound/message.go:838 | A DECIDIR |
| `midia` | internal/outbound/message.go:868 | A DECIDIR |
| `reacao` | internal/outbound/message.go:897 | A DECIDIR |
| `localizacao` | internal/outbound/message.go:901 | A DECIDIR |
| `contatos` | internal/outbound/message.go:905 | A DECIDIR |
| `flow` | internal/outbound/message.go:909 | A DECIDIR |

**11 values.** Any string outside this list is refused (`ErrUnknownType`,
`internal/outbound/message.go:938`).
...
Direction: **ENTRADA**. Field: `tipo`, `internal/outbound/message.go:237` (json tag, nested
inside each item of `botoes_template[]`, itself `internal/outbound/message.go:559`). Values,
`switch` in `validateTemplateButtons`:

| value | arquivo:linha | english |
|---|---|---|
| `url` | internal/outbound/message.go:1182 | A DECIDIR |
| `resposta_rapida` | internal/outbound/message.go:1192 | A DECIDIR |

**2 values.** Any other string is refused (`ErrUnknownTemplateButtonType`,
`internal/outbound/message.go:1208`).

### 1.4 `tipo` — instance type (`State.Type`)

Direction: **SAIDA-RESPOSTA**. Field: `tipo`, `internal/outbound/state.go:51` (json tag, matches
...
Direction: **ENTRADA**. Field: `categoria`, `internal/outbound/message.go:627` — the `midia`
...
the rule in `internal/config/counter.go:148-156`: Meta's own values are never translated). The
...
(`TemplateHeader.Type`, json `cabecalho.tipo`, `internal/outbound/message.go:175`, ENTRADA): the
comment at `internal/outbound/message.go:164-166` states this is deliberately the SAME table as
`categoria` above — but restricted to the three categories in the `mediaHeaders` map
(`internal/outbound/message.go:1061-1065`: `imagem`, `video`, `documento` — `audio` and `sticker`
excluded, comment explains why at lines 1057-1060) — **plus one literal not from this vocabulary
at all**, `"texto"` (`internal/outbound/message.go:1085`, hardcoded, checked before the category
...
`CertificateInState.State` (`internal/outbound/state.go:356`), `ObservedValue.State`
(`internal/outbound/state.go:430`), `LeadershipInState.State` (`internal/outbound/leadership.go:220`),
`ConnectorInState.State` (`internal/outbound/ingress.go:179`) and `ExternalReachInState.State`
(`internal/outbound/external_probe.go:145`) — five different blocks of the same response,
deliberately sharing one word list (comment at `internal/outbound/leadership.go:216-219`: *"uses the
...
|---|---|---|---|
| `nunca_observado` | internal/outbound/state.go:293 (`CertNeverObserved`) | — | A DECIDIR |
...
Direction: **SAIDA-RESPOSTA**. Field: `veredito`, `internal/outbound/watchdog.go:142`. Values,
`const` block:
...
Direction: **SAIDA-RESPOSTA**. Field: `veredito`, `internal/outbound/state.go:513`. Values,
`const` block:

| value | arquivo:linha | english |
|---|---|---|
| `nao_se_aplica` | internal/outbound/state.go:474 (`= NotApplicable`, canonical definition at internal/outbound/state.go:316) | A DECIDIR |
| `aguardando` | internal/outbound/state.go:478 | A DECIDIR |
| `ok` | internal/outbound/state.go:485 | (unchanged — already English) |
| `falhando` | internal/outbound/state.go:492 | A DECIDIR |
| `expirado` | internal/outbound/state.go:495 | A DECIDIR |
...
Direction: **SAIDA-RESPOSTA**. Field: `veredito,omitempty`, `internal/outbound/health_handler.go:94`.
Measured: the field is assigned exactly ONCE in the whole package, always the same literal:

| value | arquivo:linha | english |
|---|---|---|
| `nao_se_aplica` | internal/outbound/health_handler.go:155 (`= NotApplicable`, internal/outbound/state.go:316) | A DECIDIR |

**1 value, `omitempty` on every other outcome** (the field is absent from the JSON unless the
instance's type makes it not-applicable — comment at `internal/outbound/health_handler.go:86-90`).
...
(`internal/outbound/state.go:111,276`, `docs/MIGRACAO-CONTRATO-EN.md` row 25). Each entry below is
...
(`internal/config/counter.go:278-298`), in that order:

| # | value | arquivo:linha | english |
|---|---|---|---|
| 1 | `recebidas` | internal/config/counter.go:32 | A DECIDIR |
| 2 | `entregues` | internal/config/counter.go:33 | A DECIDIR |
| 3 | `recusadas_pelo_consumidor` | internal/config/counter.go:34 | A DECIDIR |
| 4 | `numero_descartado` | internal/config/counter.go:76 | A DECIDIR |
| 5 | `conta_descartada` | internal/config/counter.go:45 | A DECIDIR |
| 6 | `alarme_perda_definitiva` | internal/config/counter.go:35 | A DECIDIR |
| 7 | `enviadas` | internal/config/counter.go:36 | A DECIDIR |
| 8 | `falhas_de_envio` | internal/config/counter.go:37 | A DECIDIR |
| 9 | `leituras_marcadas` | internal/config/counter.go:100 | A DECIDIR |
| 10 | `falhas_de_leitura` | internal/config/counter.go:101 | A DECIDIR |
| 11 | `templates_apagados` | internal/config/counter.go:121 | A DECIDIR |
| 12 | `nome_antigo_usado` | internal/config/counter.go:136 | A DECIDIR |
| 13 | `cobranca_marketing` | internal/config/counter.go:167 | A DECIDIR (prefix only — suffix is Meta's literal, see below) |
| 14 | `cobranca_utility` | internal/config/counter.go:168 | A DECIDIR (prefix only) |
| 15 | `cobranca_authentication` | internal/config/counter.go:169 | A DECIDIR (prefix only) |
| 16 | `cobranca_service` | internal/config/counter.go:170 | A DECIDIR (prefix only) |
| 17 | `cobranca_outra` | internal/config/counter.go:199 | A DECIDIR |
| 18 | `cobranca_ausente` | internal/config/counter.go:215 | A DECIDIR |
| 19 | `cobranca_cobravel` | internal/config/counter.go:232 | A DECIDIR |
...
`internal/config/counter.go:148-156`) — only the `cobranca_` prefix is a candidate for renaming;
...
|---|---|---|---|---|
| 1 | `titulo` | internal/outbound/message.go:153 | `Button.Title`, one item of `botoes[]` (`Request.Buttons`, `internal/outbound/message.go:579`) — **different field than `botao_titulo`** (`docs/MIGRACAO-CONTRATO-EN.md` row 7, `internal/outbound/message.go:587`, the `cta_url`/`lista`/`flow` single-button title) | ENTRADA |
| 2 | `indice` | internal/outbound/message.go:235 | `TemplateButtonUnion.Index`, one item of `botoes_template[]` | ENTRADA |
| 3 | `telefones` | internal/outbound/block_handler.go:102 | `BlockRequest.Phones`, body of `POST /v1/bloqueios` and `DELETE /v1/bloqueios` | ENTRADA |
...
| 3 | `instancia` | `GET /v1/estado` | internal/outbound/state_handler.go:192 | A DECIDIR |
| 4 | `serie_dias` | `GET /v1/estado` | internal/outbound/state_handler.go:219 | A DECIDIR |
| 5 | `instancia` | `GET /v1/bloqueios` | internal/outbound/block_handler.go:376 | A DECIDIR |
| 6 | `limit` | `GET /v1/bloqueios` | internal/outbound/block_handler.go:410 | (unchanged — already English) |
| 7 | `after` | `GET /v1/bloqueios` | internal/outbound/block_handler.go:419 | (unchanged — already English) |
| 8 | `before` | `GET /v1/bloqueios` | internal/outbound/block_handler.go:420 | (unchanged — already English) |
| 9 | `instancia` | `GET /v1/perfil` | internal/outbound/profile_handler.go:123 | A DECIDIR |
...
(`ProfileRequest.Instance`, `internal/outbound/profile_handler.go:175`), not a query parameter — it
...
one of them present in `internal/config/counter.go:278-298` (`KeysInDisplayOrder`), the single
source `buildCounterKeys()` derives the closed vocabulary from
(`internal/config/counter.go:306-312`). **No orphan counter exists**: a repo-wide sweep for
`IncrementCounter`/counter-writing call sites (`internal/outbound/*_test.go`,
`internal/inbound/*_test.go`, `internal/config/counter.go`, `internal/inbound/handler.go`) turned
up no literal counter key outside this list.

`nome_antigo_usado` (`CounterOldNameUsed`, `internal/config/counter.go:136`) **is one of these 19**
