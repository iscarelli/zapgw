...
// ONLY way to activate an instance (fumaca.go); this command sends
...
// diagnostico_test.go, which checks the entire output WITHOUT the token —
...
//     internal/meta/diagnostico_instagram.go). The permission is tested
...
func diagnose(args []string, out io.Writer, env environment) error {
	...
	// The SAME client and SAME host resolution fumaca.go and main.go use
