...
var zapgwMenu = []menuGroup{
	{
		...
		items: []menuItem{
			...
			{
				key:   "2",
				label: "instancia listar — what exists in this database",
				args:  []string{"instancia", "listar"},
			},
			{
				key:    "3",
				label:  "instancia mostrar — one instance in detail",
				args:   []string{"instancia", "mostrar"},
				fields: []menuField{{flag: "--slug", question: "slug"}},
			},
			{
				key:   "4",
				label: "consumidor listar — who has access to what",
	...
	},
	{
		...
		items: []menuItem{
			{
				key:      "5",
				label:    "provisionar instancia — creates it (it is born PAUSED)",
			...
			{
				key:    "6",
				label:  "provisionar consumidor — creates it and prints the token ONCE",
			...
			{
				key:    "7",
				label:  "instancia rotacionar — swaps the secret of one that already exists",
				...
				warnings: []string{
					...
					"to CLEAR callback_url (send-only instance) the menu does not work: " +
						"use `zapgw instancia rotacionar --slug <slug> --callback-url ''` on the command line.",
				},
				fields: []menuField{
			...
			{
				key:    "8",
				label:  "consumidor rotacionar — new token, the previous one dies right away",
			...
			{
				key:    "10",
				label:  "fumaca — proves the channel and ACTIVATES the instance",
				args:   []string{"fumaca"},
				writes: true,
				warnings: []string{
	...
	},
	{
		...
		items: []menuItem{
			{
				key:    "11",
				label:  "instancia pausar — webhook and sending respond 503",
				args:   []string{"instancia", "pausar"},
				writes: true,
				warnings: []string{
					"does NOT ask for confirmation, on purpose: pausing can be undone (`zapgw fumaca` turns it back on).",
		...
		items: []menuItem{
			{
				...
				// deleted instance.
				key:    "99",
				label:  "instancia remover — deletes the instance and EVERY row of it",
