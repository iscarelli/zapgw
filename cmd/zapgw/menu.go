...
const secretsWarning = "the secret is NOT typed here: it comes from the environment (ZAPGW_APP_SECRET, " +
...
var zapgwMenu = []menuGroup{
	{
		...
		items: []menuItem{
			...
			{
				key:   "2",
				label: "instance list — what exists in this database",
				args:  []string{"instance", "list"},
			},
			{
				key:    "3",
				label:  "instance show — one instance in detail",
				args:   []string{"instance", "show"},
				fields: []menuField{{flag: "--slug", question: "slug"}},
			},
			{
				key:   "4",
				label: "consumer list — who has access to what",
				args:  []string{"consumer", "list"},
			},
		},
	},
	{
		title: "CREATE and CHANGE — change configuration",
		items: []menuItem{
			{
				key:      "5",
				label:    "provision instancia — creates it (it is born PAUSED)",
				args:     []string{"provision", "instancia"},
				writes:   true,
				warnings: []string{secretsWarning, "whatever is missing from the environment is randomly generated by the command."},
				fields: []menuField{
					{flag: "--slug", question: "slug (IMMUTABLE, becomes /v1/inbound/{slug})"},
					{flag: "--waba-id", question: "waba_id"},
					{flag: "--phone-number-id", question: "phone_number_id"},
					{flag: "--numero-exibido", question: "displayed number"},
					{flag: "--timeout-ms", question: "timeout in ms", note: "ENTER = the command's default"},
					{flag: "--callback-url", question: "callback_url (https://)", note: "ENTER = send-only instance"},
					{flag: "--bundle-ca", question: "PEM file with the consumer's CA", note: "ENTER = system CA store; verification stays STRICT in both cases"},
				},
			},
			{
				key:    "6",
				label:  "provision consumidor — creates it and prints the token ONCE",
				args:   []string{"provision", "consumidor"},
				writes: true,
				fields: []menuField{
					{flag: "--nome", question: "consumer name"},
					{flag: "--instancias", question: "slugs it may use, comma-separated"},
				},
			},
			{
				key:    "7",
				label:  "instance rotate — swaps the secret of one that already exists",
				args:   []string{"instance", "rotate"},
				writes: true,
				warnings: []string{
					secretsWarning,
					"whatever does not come in the environment stays INTACT — nothing is randomly generated on rotation.",
					"to CLEAR callback_url (send-only instance) the menu does not work: " +
						"use `zapgw instance rotate --slug <slug> --callback-url ''` on the command line.",
				},
				fields: []menuField{
					{flag: "--slug", question: "slug"},
					{flag: "--callback-url", question: "new callback_url", note: "ENTER = DO NOT TOUCH the callback"},
				},
			},
			{
				key:    "8",
				label:  "consumer rotate — new token, the previous one dies right away",
				args:   []string{"consumer", "rotate"},
				writes: true,
				fields: []menuField{{flag: "--nome", question: "consumer name"}},
			},
			{
				key:    "9",
				label:  "template criar — registers a template with Meta",
				args:   []string{"template", "criar"},
				writes: true,
				fields: []menuField{
					{flag: "--slug", question: "slug of the instance that owns the template"},
					{flag: "--nome", question: "template name"},
					{flag: "--categoria", question: "category (MARKETING, UTILITY, AUTHENTICATION)"},
					{flag: "--idioma", question: "language (e.g.: pt_BR)"},
					{flag: "--componentes", question: "path to the FILE with the JSON component list"},
				},
			},
			{
				key:    "10",
				label:  "smoke — proves the channel and ACTIVATES the instance",
				args:   []string{"smoke"},
				writes: true,
				warnings: []string{
					"it SENDS A REAL MESSAGE to the number typed in — a sent message cannot be undone.",
					"it is the ONLY path that activates an instance; if it fails at any step, it stays paused.",
				},
				fields: []menuField{
					{flag: "--slug", question: "slug"},
					{flag: "--destino", question: "number that will RECEIVE the message, in E.164"},
				},
			},
		},
	},
	{
		title: "TAKE DOWN — can be undone",
		items: []menuItem{
			{
				key:    "11",
				label:  "instance pause — webhook and sending respond 503",
				args:   []string{"instance", "pause"},
				writes: true,
				warnings: []string{
					"does NOT ask for confirmation, on purpose: pausing can be undone (`zapgw smoke` turns it back on).",
					"Meta re-queues what arrives and retries for up to 36h — after that, it is lost.",
				},
				fields: []menuField{{flag: "--slug", question: "slug"}},
			},
		},
	},
	{
		...
		title: "IRREVERSIBLE — cannot be undone",
		items: []menuItem{
			{
				...
				// deleted instance.
				key:    "99",
				label:  "instance remove — deletes the instance and EVERY row of it",
				args:   []string{"instance", "remove"},
				writes: true,
				warnings: []string{
					"IRREVERSIBLE. An ACTIVE instance is refused: pause it first, check nothing broke, and only then remove it.",
					"Meta does NOT find out: while the Callback URL still points at /v1/inbound/{slug}, it gets a 404 on every delivery.",
				},
				fields: []menuField{
					{flag: "--slug", question: "slug of the instance to DELETE"},
					{
						flag: "--confirmo",
						...
						// top.
						question: "type the SLUG AGAIN to confirm",
						note:     "there is no y/N here: one key is one key, and whoever is at the wrong command presses the same one",
					},
				},
			},
		},
...
func runMenu(in io.Reader, out io.Writer, env environment, run dispatcher) error {
	...
	for {
		printMenu(out)
		choice, ok := ask(reader, out, "choose")
		if !ok {
		...
		if !found {
			fmt.Fprintf(out, "unknown option %q. Type the number of one of the lines above, or 0 to quit.\n", choice)
			continue
		}
...
func printSummary(out io.Writer, env environment) {
	...
	if err != nil {
		fmt.Fprintf(out, "summary unavailable: %v\n", err)
		return
	}
	...
	if len(list) == 0 {
		fmt.Fprintf(out, "summary: no instance registered in this database.\n")
		return
	}
	...
	fmt.Fprintf(out, "summary: %d instance(s) — %s\n", len(list), strings.Join(parts, ", "))
}
