...
// (cmd/zapgw/diagnostics.go reads it from the store by slug); these
...
// assembles the verdict (cmd/zapgw/diagnostics.go) has to say, in big
...
const (
	...
	// `cmd/zapgw/diagnostics.go` prints stayed CONDITIONAL, never
...
// sweep and of the warning `cmd/zapgw/diagnostics.go` prints means changing
...
//     measurements. The warning in diagnostics.go becomes an ASSERTION:
...
// instagram_diagnostics_test.go) can prove TODAY, with save/restore, that
...
// interpreting — whoever reads the result (`cmd/zapgw/diagnostics.go`) is
