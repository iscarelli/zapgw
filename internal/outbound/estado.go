...
// (cmd/zapgw/estado.go) showed only the counter table. Four blocks the
...
type State struct {
	...
	// definitiveOutcome (vigia.go) treats that error as CREDENTIAL REFUSED — a
	// PERMANENT and FALSE `veredito: "recusado"` on every Instagram instance,
	// never a real measurement. That is what reading vigia.go for this task
...
// config/numero.go). A single `medido_em` for both would be true about one
...
// is empty — it does ONE tick before reading (see cmd/zapgw/estado.go). The
...
// not a reuse of VerdictOK/VerdictRefused (vigia.go): the Instagram token
...
//
// CAN BE nil: `zapgw estado` (cmd/zapgw/estado.go) has no way to give the
...
func instagramTokenInState(inst config.InstanceSummary, failingSince, now time.Time) InstagramTokenInState {
	...
	if err != nil {
		...
		// comment in renovador_instagram.go.checkOne. Without SetAt
