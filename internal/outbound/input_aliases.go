// input_aliases.go — T-203 (step 2 of T-189, docs/MIGRACAO-CONTRATO-EN.md)
...
// English on purpose — see the comment on Contact in message.go) or inside
...
// --- POST /v1/messages (message.go, Request) ---
...
// PLAIN quick-reply buttons of `tipo:"botoes"` (message.go's Button
...
// registration_handler.go). `waba_id`, `phone_number_id`, `app_secret`,
...
// block_handler.go). Added by T-208 (docs/MIGRACAO-CONTRATO-EN.md
...
// translateInputOrReject runs translateTopLevel(raw, dict) and, on a
