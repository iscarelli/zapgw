...
// same sentence in ingress.go about `conector`.
//
// THE MOLD IS THE SAME AS watchdog.go AND ingress.go (ConnectorProbe), and for
...
const (
	...
	// ConnectorNotConfigured (ingress.go): "not configured" is a different
...
// ingress.go already use: one missed tick is normal noise; three in a row
...
// requires (mold of watchdog.go/ingress.go).
...
func (s *ExternalProbe) ask(ctx context.Context) (string, error) {
	...
	// `ReadyConnections *int` in ingress.go.
	var body struct {
