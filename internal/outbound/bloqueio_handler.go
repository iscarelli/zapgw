...
// internal/meta/bloqueio.go). The gateway does NOT check this window on its
...
func NewBlockHandler(
	...
	// (isolamento_test.go) reads these three arguments BY TEXT and only
...
var (
	...
	// (leituras_handler.go), so the refusal log (T-037) can pass through
...
// (mensagem.go): sending without the ninth digit would silently block
// ANOTHER number.
