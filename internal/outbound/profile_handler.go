...
// internal/meta/profile.go — the docs show only `{"success":true}`), and an
...
func (h *ProfileHandler) read(w http.ResponseWriter, r *http.Request) {
	...
	// internal/meta/profile.go) — this handler adds nothing and zeroes nothing.
...
func (h *ProfileHandler) write(w http.ResponseWriter, r *http.Request) {
	...
	// (internal/meta/profile.go) omits from the JSON to Meta every key whose
