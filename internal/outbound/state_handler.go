...
//     cache (watchdog.go), which measures at its own pace. A dashboard that
...
type StateHandler struct {
	...
	// BuildStateWithSeries (state.go) already handles both types internally
	...
	// counter-carrying handlers (block_handler.go) — an optional counter
...
func (h *StateHandler) state(w http.ResponseWriter, r *http.Request) {
	...
	// moment translateInputOrReject's callers use on the body routes,
