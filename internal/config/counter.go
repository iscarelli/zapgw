...
const (
	...
	// (internal/outbound/reads_handler.go).
	...
	//
	// 🔴 ONLY `sent` COUNTS, AND ONLY IT — internal/inbound/billing.go. The
	...
	// once per slug+category per process — see internal/inbound/billing.go).
...
// it was born in cmd/zapgw/state.go, and T-060 added a SECOND reader (the
...
// estado` (cmd/zapgw/state.go) and the `GET /v1/estado` route
// (internal/outbound/state_handler.go) call THIS function, never a copy
