...
// internal/config/transito.go). The FOUR SPELLINGS of the same subscriber
...
// discipline as IncrementCounter (contador.go) — a value outside
...
// (contador_test.go) — Record returns nothing, so the only possible
...
// (T-131) proves the comment on ClearTransitByPhone (transito.go,
