...
// request, 2026-07-29): the counters (contador.go) answer "how many
...
// as ErrUnknownCounterKey (contador.go): a value outside the
...
// CounterRetentionDays (contador.go): read ONCE, in `main`, and the
...
// responsibility as IncrementCounter vs. Counter (contador.go).
...
// (cmd/zapgw/transito.go), so echoing the counterpart back adds no
...
type TransitLine struct {
	...
	// from, and this field is where. cmd/zapgw/transito.go prints "—" for
...
// technique as CounterStore (contador.go).
...
//
// The SAME guarantee as Counter (contador.go), and for the SAME reason:
