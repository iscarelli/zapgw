...
// (internal/config/transito.go, internal/meta/phone.go). T-161 swapped
...
//     internal/config/transito_test.go).
...
// cmd/zapgw/log_test.go, cmd/zapgw/transito_test.go,
// internal/config/transito.go, internal/config/transito_test.go,
// internal/meta/phone.go, internal/meta/phone_test.go,
// internal/outbound/transito_test.go) was swapped for 5532999990000 in
...
// the synthetic wamid in internal/config/transito_test.go, a second
...
func TestNoPhoneNumberOutsideTheAllowlistInTheRepo(t *testing.T) {
	...
	if len(outsideTheAllowlist) > 0 {
		...
		t.Fatalf("numero(s) fora da allowlist encontrado(s):\n%s\n\n"+
			...
			"(internal/config/telefones_allowlist_test.go) com o porque. "+
	...
	for _, required := range []string{
		"internal/config/transito.go", // production — had the real number
...
// used in internal/config/transito_test.go,
